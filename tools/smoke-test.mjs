...
// 5. un examen lancé depuis un raccourci de série tire aussi au sort
await nav('#/examen/serie-07');
ok('Examen lancé depuis la série 7', window.document.getElementById('qcard').innerHTML.includes('question-text'));
ok('Examen tiré au sort (étiquette « Examen blanc »)', app.innerHTML.includes('Examen blanc'));
...
// 6bis. deux examens consécutifs ne posent pas les mêmes questions
{
  const hist = JSON.parse(window.localStorage.getItem('pm.historique') || '[]');
  const ids1 = (hist[1] && hist[1].ids) || [];
  const ids2 = (hist[0] && hist[0].ids) || [];
  const shared = ids1.filter((id) => ids2.includes(id)).length;
  ok('Questions différentes à chaque tentative', ids1.length === 40 && ids2.length === 40 && shared === 0,
    shared + ' question(s) commune(s) entre deux examens consécutifs');
}
...
// 9. page des examens blancs
await nav('#/series');
ok('Page examens : bouton de lancement', app.innerHTML.includes('Commencer un examen blanc'));
ok('Page examens : tirage au sort expliqué', app.innerHTML.includes('au sort'));
ok('Page examens : seuil 32/40 affiché', app.innerHTML.includes('32/40'));
