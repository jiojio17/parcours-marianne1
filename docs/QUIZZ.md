...
Les examens blancs sont **tirés au sort à chaque lancement**, côté navigateur, par
`app/app.js`, en respectant la répartition officielle (28 connaissances + 12 mises en
situation). Pour éviter la mémorisation d'un questionnaire figé, chaque nouvelle
tentative évite les questions vues récemment : l'application conserve la liste des
questions déjà posées (localStorage) et ne les réutilise qu'une fois le reste de la
banque parcouru. Résultat : **deux essais consécutifs ne posent jamais les mêmes
questions**, tant que la banque le permet.

Les 40 « séries » numérotées, générées à l'origine avec une graine fixe (elles posaient
donc toujours les mêmes questions), ont été supprimées : c'était ce défaut. Le fichier
`app/data/series.js` ne contient plus de tirage figé.
